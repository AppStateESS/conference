...
use conference\Exception\BadCommand;
use conference\Exception\PrivilegeMissing;
use conference\Factory\StoryMenu;
use phpws2\Database;
use Canopy\Request;
...
abstract class SubController
{
    ...
    protected function loadRequestId(Request $request)
    {
        ...
        if (!is_numeric($id)) {
            throw new \conference\Exception\ResourceNotFound($id);
        }
        $this->id = $id;
    }
